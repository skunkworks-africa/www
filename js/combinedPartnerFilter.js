...
// Function to fetch and add icons
function fetchAndAddIcons() {
    const iconPath = 'https://raw.githubusercontent.com/skunkworksza/www/dc3276070f277756eceb712d3027dd8ba85ecccb/assets/icons/';
    ...
    fetch(iconPath)
        ...
        .then(data => {
            ...
            const icons = doc.querySelectorAll('a');
            const iconUrls = Array.from(icons).map(icon => `${iconPath}${icon.getAttribute('href')}`);

            iconUrls.forEach((url, index) => {
                const iconName = url.split('/').pop().split('.')[0];
                const iconImg = `<img src="${url}" alt="${iconName} Icon" class="icon">`;

                // Add the icon to necessary places
                const elements = document.querySelectorAll(`.${iconName}`);
                elements.forEach(element => {
                    element.innerHTML = iconImg + element.innerHTML;
                });
            });
        })
...
// Initialize partners and icons on page load
